...
export const authTokenInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  return from(Preferences.get({ key: 'fs-token' })).pipe(
    switchMap((token) => { // switchMap is necessary if it returns another observable or a promise
      console.log("ESTE ES TU TOOOOOOOOKKKEEEEEEEEN  PERRRRRAAAAAAAAAAAAA",token);
      console.log("oooooooooooobbbbbbbbbbbbbjjjjjjjjjeeeeeeeeetttttttttooooo   preferencesss",Preferences);
      if (!token.value) {
        return next(req);
      }
